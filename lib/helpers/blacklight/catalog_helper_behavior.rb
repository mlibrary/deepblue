...
module Blacklight::CatalogHelperBehavior
  ...
  def query_label
    puts "query_label"
    return "" if search_state.search_field&.key.blank?
    return "" if default_search_field2?(search_state.search_field.key)
    label_for_search_field(search_state.search_field.key)
  end
