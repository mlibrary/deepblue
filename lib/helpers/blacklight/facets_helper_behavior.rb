...
# require File.join( Gem::Specification.find_by_name("blacklight").full_gem_path, "app/controllers/concerns/blacklight/facet.rb" )

require_relative '../../controllers/concerns/blacklight/facet'
...
module Blacklight::FacetsHelperBehavior
  ...
  # @return [Boolean]
  def has_facet_values? fields = facet_field_names(nil), response = nil
    ...
    begin # Deprecation.silence(Blacklight::FacetsHelperBehavior) do
      begin # Deprecation.silence(Blacklight::Facet) do
        puts "has_facet_values? facets..."
        facets = facets_from_request2(fields, response)
        facets.any? { |display_facet| should_render_facet?(display_facet) }
  ...
  def render_facet_partials fields = nil, options = {}
    ...
    begin # Deprecation.silence(Blacklight::FacetsHelperBehavior) do
      begin # Deprecation.silence(Blacklight::Facet) do
        safe_join(facets_from_request2(fields, response).map do |display_facet|
  ...
  def render_facet_limit_list2(paginator, facet_field, wrapping_element = :li)
    facet_config ||= facet_configuration_for_field(facet_field)
  ...
  def should_render_facet? display_facet, facet_config = nil
    puts "should_render_facet? #{display_facet}"
