...
END_OF_SCHEDULER_ENTRY
  ...
  class FileExportChecksumReportTask < ::DataDen::AbstractReportTask

    mattr_accessor :file_export_report_checksum_task_debug_verbose, default: false

    def initialize( msg_handler: nil, options: {} )
      super( msg_handler: msg_handler, options: options )
      @event = "FileSet Checksum Report"
      if msg_handler.nil?
        @verbose = true
      ...
      @email_subject = "All DataDen file export checksums on %hostname% as of %now%"
    ...
    def report_file_init( default_value: "%date%.%hostname%.file_export_checksum_report.csv" )
    ...
    def run_report
      ...
      records.each do |record|
        ...
        next if record.checksum_value.blank?
