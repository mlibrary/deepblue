...
RSpec.feature 'Create a DataSet', type: :feature, js: true, workflow: true, clean_repo: true, skip: Rails.configuration.hyrax4_spec_skip || ENV['CIRCLECI'].present? do
  ...
  context 'a logged in user can edit their own work', skip: true do
    ...
    let(:other_user_attributes) do
      { email: 'test2@example.com' }
    end
    let(:other_user) do
      User.new(other_user_attributes) { |u| u.save(validate: false) }
    end
    ...
    scenario do
      ...
      click_link( 'Edit Work/Add Files', wait: wait_after_click )
