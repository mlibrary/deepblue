RSpec.describe 'Creating a new Work', :js, :workflow do
  ...
  xcontext "when the user is not a proxy" do
    before do
      # sign_in user
      login_as user
      click_link 'Works'
  ...
  xcontext 'when the user is a proxy', perform_enqueued: [ContentDepositorChangeEventJob, AttachFilesToWorkJob, IngestJob] do
  ...
  xcontext "when a file uploaded and then deleted" do
    before do
      # sign_in user
      login_as user
      click_link 'Works'
