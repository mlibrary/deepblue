...
RSpec.describe Hyrax::Actors::EnvironmentEnhanced, skip: false do

  let(:user)    { factory_bot_create_user(:user) }
  let(:work)    { create(:data_set, user: user) }
  let(:ability) { Ability.new(user) }

  it { expect( Hyrax::Actors::EnvironmentEnhanced::ENVIRONMENT_ENHANCED_DEBUG_VERBOSE ).to eq false }
