...
FactoryBot.define do
  factory :generic_work, aliases: [:private_generic_work], class: GenericWork do
    ...
    factory :generic_work_with_one_child do
      before(:create) do |work, evaluator|
        work.ordered_members << create(:generic_work, user: evaluator.user, title: ['A Contained Work'])
      end
    end

    factory :generic_work_with_two_children do
      before(:create) do |work, evaluator|
        work.ordered_members << create(:generic_work, user: evaluator.user, title: ['A Contained Work'], id: "BlahBlah1")
        work.ordered_members << create(:generic_work, user: evaluator.user, title: ['Another Contained Work'], id: "BlahBlah2")
      end
    end

    factory :generic_work_with_representative_file do
      before(:create) do |work, evaluator|
    ...
    factory :generic_work_with_file_and_work do
      before(:create) do |work, evaluator|
        work.ordered_members << create(:file_set, user: evaluator.user)
        work.ordered_members << create(:generic_work, user: evaluator.user)
      end
    end

    factory :generic_work_with_embargo_date do
      # build with defaults:
      # let(:generic_work) { create(:embargoed_work) }
      ...
      # end
      # let(:generic_work) { create(:embargoed_work, with_embargo_attributes: embargo_attributes) }
    ...
    factory :generic_work_with_lease_date do
      # build with defaults:
      # let(:generic_work) { create(:leased_work) }
      ...
      # end
      # let(:generic_work) { create(:leased_work, with_lease_attributes: lease_attributes) }
