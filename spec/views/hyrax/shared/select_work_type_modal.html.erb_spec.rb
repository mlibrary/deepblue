...
RSpec.describe 'shared/_select_work_type_modal.html.erb', type: :view do
  ...
  context 'when no collections id' do
    ...
    it 'draws the modal' do
      ...
      expect(rendered).to have_content 'Data Set'
      # expect(rendered).to have_content 'Nested Work'
      expect(rendered).to have_selector 'input[type="radio"][data-single="/concern/data_sets/new"][data-batch="/batch_uploads/new?payload_concern=DataSet"]'
  ...
  context 'when collection id exists' do
    ...
    it 'draws the modal with collection id' do
      expect(rendered).to have_selector 'input[type="radio"][data-single="/concern/data_sets/new?add_works_to_collection=1"][data-batch="/batch_uploads/new?add_works_to_collection=1&payload_concern=DataSet"]' # rubocop:disable Metrics/LineLength
