...
describe "yaml_update_for_works.rake" do

  # reference: https://tasdikrahman.me/2020/10/20/testing-rake-tasks-with-rspec/

  context "yaml_update_for_works" do

    let(:task)    { 'deepblue:yaml_update_for_works' }
    let(:options) { { source_dir: "/deepbluedata-prep", ingester: "ingester@umich.edu" } }
    let(:ids)     { 'f4752g72m f4752g72m' }
    let(:invoked) { ::Deepblue::YamlUpdateForWorks.new( ids: ids, options: options ) }


    before do
      expect(::Deepblue::YamlUpdateForWorks).to receive(:new)
    ...
    it "invokes Deepblue::YamlUpdateForWorks" do
