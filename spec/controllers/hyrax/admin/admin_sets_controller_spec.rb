...
RSpec.describe Hyrax::Admin::AdminSetsController, :clean_repo, skip: false do
  ...
  let(:creator) { factory_bot_create_user(:user, email: 'creator@example.com') }
  ...
  context "a general registered user" do
    ...
    describe "#show" do
      #hyrax5 - before { controller.instance_variable_set(:@admin_set, admin_set) }
      ...
      context "when user has access through public group" do
        ...
        #hyrax5 - let(:admin_set) { create(:adminset_lw, with_solr_document: true, with_permission_template: { view_groups: ['public'] }) }
        let(:admin_set) do
      ...
      context "when user has access through registered group" do
        ...
        # it on the admin page.
        #hyrax5 - let(:admin_set) { create(:adminset_lw, with_solr_document: true, with_permission_template: { view_groups: ['registered'] }) }
        let(:admin_set) do
          valkyrie_create(:hyrax_admin_set,
                          with_permission_template: true,
                          access_grants: [{ agent_type: Hyrax::PermissionTemplateAccess::GROUP,
      ...
      context "when user is directly granted view access" do
        # Even though the user can view this admin set, the should not be able to view
        # it on the admin page.
        #hyrax5 - let(:admin_set) { create(:adminset_lw, with_solr_document: true, with_permission_template: { view_users: [user.user_key] }) }
        let(:admin_set) do
          valkyrie_create(:hyrax_admin_set,
                          with_permission_template: true,
    ...
    describe "#edit" do
      ...
      context "when user is directly granted manage access" do
        # Even though the user can view this admin set, the should not be able to view
        # it on the admin page.
        #hyrax5 - let(:admin_set) { create(:adminset_lw, with_solr_document: true, with_permission_template: { manage_users: [user.user_key] }) }
        #hyrax5 - let(:admin_set2) { create(:adminset_lw, with_solr_document: true, with_permission_template: true) }
        let(:admin_set) do
          valkyrie_create(:hyrax_admin_set,
                          with_permission_template: true,
                          access_grants: [{ agent_type: Hyrax::PermissionTemplateAccess::USER,
        ...
        context 'and user is accessing the managed set' do
          it 'defines a form' do
            ...
            expect(response).to be_successful
            #hyrax5 - expect(assigns[:form]).to be_kind_of Hyrax::Forms::AdminSetForm
            if Rails.configuration.hyrax_disable_wings || Hyrax.config.disable_wings
  ...
  shared_examples('specs for varied user types') do
    ...
    describe "#create" do
      context "when it's successful" do
        ...
        it 'creates admin set', skip: Rails.configuration.hyrax5_spec_skip do
          post :create, params: { admin_set: { title: 'Test title',
                                               description: 'test description',
                                               workflow_name: 'default' } }
          # TODO: the path generated does not include 'data/'
