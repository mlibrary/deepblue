...
RSpec.configure do |config|
  config.before(datacite_api: true) do
    ...
    stub_request(:delete, URI.join(::Deepblue::DoiMintingService.test_mds_base_url, "doi/#{prefix}/findable-doi"))
      .with(basic_auth: ['username', 'password'])
      .to_return(status: 405, body: '{"errors":[{"status":"405", "title":"Method not allowed"}]}')

    # GET <MDS_BASE>/metadata/<prefix>/draft-doi
    # Get doi metadata
    stub_request(:get, URI.join(::Deepblue::DoiMintingService.test_mds_base_url, "metadata/#{prefix}/draft-doi"))
      .with(basic_auth: ['username', 'password'])
    ...
    stub_request(:get, URI.join(::Deepblue::DoiMintingService.test_mds_base_url, "metadata/#{prefix}/unknown-doi"))
      .with(basic_auth: ['username', 'password'])
      .to_return(status: 404, body: 'DOI is unknown to MDS')

    # PUT <MDS_BASE>/metadata/<prefix>
    # Create new draft doi with metadata
    stub_request(:put, URI.join(::Deepblue::DoiMintingService.test_mds_base_url, "metadata/#{prefix}"))
    ...
    stub_request(:put, URI.join(::Deepblue::DoiMintingService.test_mds_base_url, "metadata/#{prefix}/draft-doi"))
      .with(headers: { 'Content-Type': 'application/xml;charset=UTF-8' },
            basic_auth: ['username', 'password'])
      .to_return(status: 201, body: "OK (#{prefix}/draft-doi)")
    ...
    # Update metadata for draft doi
    stub_request(:delete, URI.join(::Deepblue::DoiMintingService.test_mds_base_url, "metadata/#{prefix}/draft-doi"))
      .with(basic_auth: ['username', 'password'])
      .to_return(status: 200, body: "OK")

    # GET <MDS_BASE>/doi/<prefix>/draft-doi
    # Get doi url
    stub_request(:get, URI.join(::Deepblue::DoiMintingService.test_mds_base_url, "doi/#{prefix}/draft-doi"))
      .with(basic_auth: ['username', 'password'])
      .to_return(status: 200, body: "https://www.moomin.com/en/")

    # GET <MDS_BASE>/doi/<prefix>/unknown-doi
    # Get doi url with unknown doi
    stub_request(:get, URI.join(::Deepblue::DoiMintingService.test_mds_base_url, "doi/#{prefix}/unknown-doi"))
      .with(basic_auth: ['username', 'password'])
      .to_return(status: 404, body: "DOI is unknown to MDS")

    # PUT <MDS_BASE>/doi/<prefix>/draft-doi
    # Update url for draft doi
    stub_request(:put, URI.join(::Deepblue::DoiMintingService.test_mds_base_url, "doi/#{prefix}/draft-doi"))
    ...
    stub_request(:put, URI.join(::Deepblue::DoiMintingService.test_mds_base_url, "doi/#{prefix}/unknown-doi"))
      .with(headers: { 'Content-Type': 'text/plain;charset=UTF-8' },
            basic_auth: ['username', 'password'])
