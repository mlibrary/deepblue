...
RSpec.describe Hyrax::Orcid::Profile::FundingPresenter do
  let(:service) { described_class.new(reader.read_funding) }
  ...
  describe "#key" do
    it { expect(service.key).to eq "funding" }
  end
  ...
  describe "#collection" do
    ...
    it "returns a hash" do
      ...
      expect(result.first.dig(:title)).to eq "Grant title"
      expect(result.first.dig(:items).first).to eq "Wellcome Trust (London)"
    end
  end
end
