...
RSpec.describe Hyrax::Orcid::Profile::PeerReviewPresenter do
  let(:service) { described_class.new(reader.read_peer_reviews) }
  ...
  describe "#key" do
    it { expect(service.key).to eq "peer_reviews" }
  end
  ...
  describe "#collection" do
    ...
    it "returns a hash" do
      ...
      expect(result.first.dig(:title)).to be_nil
      expect(result.first.dig(:items).first).to eq "ORCID"
    end
  end
end
