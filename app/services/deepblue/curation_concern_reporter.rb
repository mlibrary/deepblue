  ...
  class CurationConcernReporter < AbstractService
    ...
    protected
      ...
      def process_collection( collection:, report_line_prefix: '' )
        ...
        # console_print "#{report_line_prefix}[#{collection.id}] has #{works( work_ids.count )} and #{files( file_set_count )}..."
        console_print "#{report_line_prefix}[#{collection.id}] has #{works(work_ids.count )} ..."
        work_ids.each do |wid|
          ...
          if work_ids_reported.key?( wid )
            console_print " already reported as"
          ...
          console_print " #{human_readable( @work_size )} in #{files( @work_file_count )}"
        end
      ...
      def process_collections
        ...
        console_puts "#{collections.size} collections to process..."
        count = 0
      ...
      def process_work( work:, report_line_prefix: '' )
        ...
        @work_size = 0
        console_print "#{report_line_prefix}#{work.id} has #{files(work.file_set_ids.size)}..."
        if work_ids_reported.key?( work.id )
          console_print " already reported ..."
        ...
        console_print " #{human_readable( @work_size )}\n"
        print_work_line( out_works, work: work, work_size: @work_size )
      end

      def process_works
        works = TaskHelper.all_works
        console_puts "#{works.size} works to process..."
        count = 0
        works.each do |work|
          count += 1
      ...
      def report_finished
        ...
        console_puts "\n"
        console_puts out_report.string
      end
