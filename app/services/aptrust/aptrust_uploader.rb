...
class Aptrust::TarBag
  ...
  def run
    ...
    msg_handler.bold_debug [ msg_handler.here, msg_handler.called_from,
                             ...
                             "tar_target_file=#{tar_target_file}",
                             "source_dir_name=#{source_dir_name}",
                             "" ] if msg_handler.debug_verbose
    tar_cmd = "cd #{operation_dir};tar -cf #{tar_target_file} #{source_dir_name}"
    msg_handler.bold_debug [ msg_handler.here, msg_handler.called_from,
                             "tar_cmd=#{tar_cmd}",
                             "" ] if msg_handler.debug_verbose
    @tar_cmd_out = `#{tar_cmd}`
    msg_handler.bold_debug [ msg_handler.here, msg_handler.called_from,
                             "@tar_cmd_out=#{@tar_cmd_out}",
                             "" ] if msg_handler.debug_verbose
    return @tar_cmd_out
  end
...
class Aptrust::AptrustUploader
  ...
  def aptrust_info_write( bag:, aptrust_info: nil, note: nil )
    msg_handler.bold_debug [ msg_handler.here, msg_handler.called_from,
                             "bag_id=#{id_from( bag: bag )}",
                             "note=#{note}",
                             "aptrust_info=#{aptrust_info}",
                             "aptrust_info.present?=#{aptrust_info.present?}",
                             "self.aptrust_info=#{self.aptrust_info}",
                             "self.aptrust_info.present?=#{self.aptrust_info.present?}" ] if debug_verbose
    aptrust_info ||= self.aptrust_info
    msg_handler.bold_debug [ msg_handler.here, msg_handler.called_from,
                             "aptrust_info=#{aptrust_info}",
                             "aptrust_info.present?=#{aptrust_info.present?}",
                             "aptrust_info.respond_to? :build=#{aptrust_info.respond_to? :build}" ] if debug_verbose
    aptrust_info = aptrust_info.build( msg_handler: msg_handler ) if aptrust_info.respond_to? :build
    msg_handler.bold_debug [ msg_handler.here, msg_handler.called_from,
                             "aptrust_info=#{aptrust_info}" ] if debug_verbose
    file = File.join( bag.bag_dir, BAG_FILE_APTRUST_INFO )
    msg_handler.bold_debug [ msg_handler.here, msg_handler.called_from,
                             "file=#{file}" ] if debug_verbose
  ...
  def bag_export_data_file_sets( bag:, file_sets: )
    ...
    begin # until true for break
      ...
      # TODO: use more general export
      export_data_by_closure( data_dir, file_sets )
  ...
  def bag_manifest( bag: )
    ...
    begin # until true for break
      ...
      begin
        ...
        unless e.messaage.starts_with? "Tag file already in manifest:"
  ...
  def bag_pack( bag:, note: nil )
    ...
    begin # until true for break
      ...
      if use_external_tar_cmd
        ...
        msg_handler.bold_debug [ msg_handler.here, msg_handler.called_from, "using Minitar"] if debug_verbose
        parent = File.dirname bag.bag_dir
        Dir.chdir( parent ) do
          tar_src = File.basename bag.bag_dir
          tar_filename = tar_filename( bag: bag )
          msg_handler.bold_debug [ msg_handler.here, msg_handler.called_from,
                                   "tar_src=#{tar_src}",
                                   "tar_filename=#{tar_filename}"] if debug_verbose
      ...
      if "" != `which apt-cmd`
        ...
        rv = `#{cmd}`
        msg_handler.msg "  #{cmd}"
        msg_handler.msg "  returned: '#{rv.chomp}'"
      end
  ...
  def bag_pack_and_upload_multibag( base_bag_id:, files: )
    # TODO: add support for event_start and event_stop
    msg_handler.bold_debug [ msg_handler.here, msg_handler.called_from, "base_bag_id=#{base_bag_id}" ] if debug_verbose
    begin # until true for break
      partitioned = []
      files.sort_by_size
      ...
      begin
        ...
        files.delete_file_sets( file_set_list: partition )
  ...
  def bag_tar_mv( bag:, export_tar_file:, note: nil )
    ...
    msg_handler.bold_debug [ msg_handler.here, msg_handler.called_from,
                             ...
                             "tar_file=#{tar_file}",
                             "note=#{note}",
                             "moving tar_file to export_tar_file"] if debug_verbose
  ...
  def bag_upload( bag:, note: nil )
    ...
    begin # until true for break
      ...
      begin
        ...
        msg = "failed in #{e.backtrace[0]} with error #{e}"
        export_failed( status: ::Aptrust::EVENT_UPLOAD_FAILED, note: msg )
  ...
  def deposit
    ...
    if 0 == total_size
      ...
      msg_handler.bold_error [ msg_handler.here, msg_handler.called_from, msg, "" ]
    elsif total_size < bag_max_total_file_size
      deposit_single_bag()
    else
      deposit_multibag()
    end
  ...
  def deposit_files
    # by default, we don't worry about files and let the exporter figure it out
    return []
  end
  ...
  def deposit_multibag
    ...
    begin
      ...
      export_failed( status: ::Aptrust::EVENT_DEPOSIT_FAILED, note: "failed in #{e.backtrace[0]} with error #{e}" )
    end
    msg_handler.bold_debug [ msg_handler.here, msg_handler.called_from,
                             "@most_recent_status=#{@most_recent_status}" ] if debug_verbose
  end
  ...
  def deposit_single_bag
    ...
    begin
      ...
      exporting( bag: bag )
      exported( bag: bag )
      bag_pack_and_upload_single( bag: bag )
      deposited( bag: bag )
  ...
  def export_data_by_closure( data_dir, files )
    ...
    export_by_closure.call( data_dir, files )
    msg_handler.bold_debug [ msg_handler.here, msg_handler.called_from, "data_dir=#{data_dir}" ] if debug_verbose
  end

  def export_data_by_copy( data_dir, files )
    # TODO: files
    msg_handler.bold_debug [ msg_handler.here, msg_handler.called_from, "data_dir=#{data_dir}" ] if debug_verbose
  ...
  def export_data_by_move( data_dir, files )
    # TODO: files
    msg_handler.bold_debug [ msg_handler.here, msg_handler.called_from, "data_dir=#{data_dir}" ] if debug_verbose
    return if export_src_dir.nil?
    return unless File.directory? export_src_dir
  ...
  def export_failed( note:, status: ::Aptrust::EVENT_FAILED )
    msg_handler.msg_error note
    note = note.first if note.is_a? Array
    track_with_sleep( status: status, error: true, note: note )
  end
  ...
  def track_with_sleep( status:, note: nil, error: false, sleep_secs: event_sleep_secs )
    ...
    begin # until true for break
      ...
      sleep( sleep_secs )
      if msg_handler.verbose
        status_str = "#{DeepblueHelper.display_timestamp DateTime.now}: #{@object_id}: #{status}"
        pad_size = (::Aptrust::EVENT_DEPOSITING.size - status.size)
        status_str += " " * pad_size if 0 < pad_size
        status_str += " -- #{note}" if note.present?
        msg_handler.msg_verbose status_str
      end
      aptrust_upload_status.track( status: status, note: note )
    end until true # for break
    @most_recent_status = status
  ...
  def upload_legacy( filename:, id: 'uknown' )
    ...
    begin
      ...
      export_failed( status: ::Aptrust::EVENT_UPLOAD_FAILED, note: "failed in #{e.backtrace[0]} with error #{e}" )
