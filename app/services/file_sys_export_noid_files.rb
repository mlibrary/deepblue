...
class FileSysExportNoidFiles
  ...
  def initialize( export_service:, file_sys_export: )
    ...
    @files = init_files( msg_handler: export_service.msg_handler )
    init_maps( msg_handler: export_service.msg_handler )
  end

  def init_files( msg_handler: )
  ...
  def init_maps( msg_handler: )
    ...
    msg_handler.bold_debug [ msg_handler.here, msg_handler.called_from, "@files.size=#{@files.size}" ] if msg_handler.debug_verbose
    @files.each do |file_rec|
  ...
  def add( fs: )
    ...
    file_rec = FileExport.find_or_create( file_sys_export: file_sys_export, fs: fs )
    ...
    @files << file_rec
    @fsid_map[ file_rec.noid ] = file_rec
    add_export_file_name_to_file_rec( file_set: fs, file_rec: file_rec, save: true )
  ...
  def add_export_file_name_to_file_rec( file_set:, file_rec:, save: true )
  ...
  def export_file_name_for( file_set:, file_rec:, save: true )
    rv = FileSysExportService.export_file_name_for( file_sys_export_noid_files: self,
                                                    file_set: file_set,
                                                    file_rec: file_rec,
                                                    save: save )
