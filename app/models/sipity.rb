...
# monkey # from hyrax/app/models/sipity.rb
##
...
module Sipity
  ...
  mattr_accessor :model_sipity_debug_verbose, default: false # monkey #
  ...
  def Entity(input, &block) # rubocop:disable Metrics/AbcSize
    Hyrax.logger.debug("Trying to make an Entity for #{input.inspect}") if model_sipity_debug_verbose  # monkey #

    result = case input
             when Sipity::Entity
               input
             when URI::GID, GlobalID
               Hyrax.logger.debug("Entity() got a GID, searching by proxy") if model_sipity_debug_verbose  # monkey #
               Entity.find_by(proxy_for_global_id: input.to_s)
             when SolrDocument
               Hyrax.logger.debug("Entity() got a SolrDocument, retrying on #{input.to_model}") if model_sipity_debug_verbose  # monkey #
               Entity(input.to_model)
             when Draper::Decorator
               Hyrax.logger.debug("Entity() got a Decorator, retrying on #{input.model}") if model_sipity_debug_verbose  # monkey #
               Entity(input.model)
             when Sipity::Comment
               Hyrax.logger.debug("Entity() got a Comment, retrying on #{input.entity}") if model_sipity_debug_verbose  # monkey #
               Entity(input.entity)
             when Valkyrie::Resource
               Hyrax.logger.debug("Entity() got a Resource, retrying on #{Hyrax::GlobalID(input)}") if model_sipity_debug_verbose  # monkey #
               Entity(Hyrax::GlobalID(input))
             else
               Hyrax.logger.debug("Entity() got something else, testing #to_global_id") if model_sipity_debug_verbose  # monkey #
               Entity(input.to_global_id) if input.respond_to?(:to_global_id)
             end

    Hyrax.logger.debug("Entity(): attempting conversion on #{result}") if model_sipity_debug_verbose  # monkey #
