...
module Sipity
  ...
  class Workflow < ActiveRecord::Base
    ...
    def self.find_active_workflow_for(admin_set_id:)
      ...
      # ::Deepblue::LoggingHelper.bold_debug [ ::Deepblue::LoggingHelper.here,
      #                                        ::Deepblue::LoggingHelper.called_from,
      #                                        "templates=#{templates.pretty_inspect}",
      #                                        "" ] if debug_verbose
      # # perm_temp = Hyrax::PermissionTemplate.where( source_id: admin_set_id )
      # # ::Deepblue::LoggingHelper.bold_debug [ ::Deepblue::LoggingHelper.here,
      # #                                        ::Deepblue::LoggingHelper.called_from,
      # #                                        "perm_temp=#{perm_temp.pretty_inspect}",
      # #                                        "" ] if debug_verbose
      workflows = Sipity::Workflow.arel_table
      # ::Deepblue::LoggingHelper.bold_debug [ ::Deepblue::LoggingHelper.here,
      #                                        ::Deepblue::LoggingHelper.called_from,
      #                                        "workflows=#{workflows.pretty_inspect}",
      #                                        "" ] if debug_verbose
      # # ::Deepblue::LoggingHelper.bold_debug [ ::Deepblue::LoggingHelper.here,
      # #                                        ::Deepblue::LoggingHelper.called_from,
      # #                                        "workflows[:permission_template_id]=#{workflows[:permission_template_id].pretty_inspect}",
      # #                                        "templates[:id]=#{templates[:id].pretty_inspect}",
      # #                                        "templates[:source_id]=#{templates[:source_id].pretty_inspect}",
      # #                                        "" ] if debug_verbose
      # qr = Sipity::Workflow.where(
      ...
      #                                          ::Deepblue::LoggingHelper.called_from,
      #                                          "r=#{r.pretty_inspect}",
      #                                          "" ] if r.active && debug_verbose
      #   return r if r.active
      # end
      # hyrax5 update: for some reason, this query is broken
      # Sipity::Workflow.where(active: true).where(
      Sipity::Workflow.where.not(active: nil).where(
        workflows[:permission_template_id].in(
          templates.project(templates[:id]).where(templates[:source_id].eq(admin_set_id.to_s))
        )
      ).first!
      ...
      #                                        ::Deepblue::LoggingHelper.called_from,
      #                                        "admin_set_id=#{admin_set_id}",
      #                                        "ActiveRecord::RecordNotFound.err=#{err}",
      #                                        "" ] if true
    ...
    def self.activate!(permission_template:, workflow_id: nil, workflow_name: nil)
      ...
      active_workflow = Sipity::Workflow.where(permission_template: permission_template).where.not(active: nil)
