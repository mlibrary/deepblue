...
class DoiMintingJob < ::Hyrax::ApplicationJob
  ...
  mattr_accessor :doi_minting_job_debug_verbose
  @@doi_minting_job_debug_verbose = false
  ...
  def perform( id, current_user: nil, job_delay: 0, target_url: )
    ...
    # Rails.logger.debug "DoiMintingJob curation_concern id #{id} #{user.email} starting..."
    if ::Deepblue::DoiMintingService.mint_doi_for( curation_concern: curation_concern,
                                                 current_user: current_user,
                                                 target_url: target_url )
      Rails.logger.debug "DoiMintingJob curation_concern id #{id} #{user.email} succeeded."
      # do success callback
      ...
      Rails.logger.debug "DoiMintingJob curation_concern id #{id} #{user.email} failed."
      # do failure callback
