class MultiValueInput < SimpleForm::Inputs::CollectionInput
  ...
  mattr_accessor :multi_value_input_debug_verbose, default: false
  ...
  private
    ...
    def build_field_options(value, index)
      ::Deepblue::LoggingHelper.bold_debug [ ::Deepblue::LoggingHelper.here, ::Deepblue::LoggingHelper.called_from,
                                             "value=#{value}",
                                             "index=#{index}",
                                             "" ] if multi_value_input_debug_verbose
      options = input_html_options.dup

      options[:value] = value
      #options[:addValue] = "Add one more..."
    ...
    def build_field(value, index)
      # ::Deepblue::LoggingHelper.bold_debug [ ::Deepblue::LoggingHelper.here, ::Deepblue::LoggingHelper.called_from, "in gem" ] if multi_value_input_debug_verbose
      ::Deepblue::LoggingHelper.bold_debug [ ::Deepblue::LoggingHelper.here, ::Deepblue::LoggingHelper.called_from,
                                             "value=#{value}",
                                             "index=#{index}",
                                             "" ] if multi_value_input_debug_verbose
      ...
      if options.delete(:type) == 'textarea'.freeze
        rv = @builder.text_area(attribute_name, options)
      else
        rv = @builder.text_field(attribute_name, options)
      end
      ::Deepblue::LoggingHelper.bold_debug [ ::Deepblue::LoggingHelper.here, ::Deepblue::LoggingHelper.called_from,
